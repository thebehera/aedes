...
function Client (broker, conn) {
  ...
  this.deliver0 = function deliverQoS0 (_packet, cb) {
    var toForward = dedupe(_packet) &&
      that.broker.authorizeForward(_packet)
    console.log('deliver 0, forward', _packet.topic, JSON.parse(_packet.payload))
    if (toForward) {
  ...
  this.deliverQoS = function deliverQoS (_packet, cb) {
    ...
    if (_packet.qos === 0) {
      ...
      console.log('deliver qos 1, forward', _packet.topic, JSON.parse(_packet.payload))
      if (toForward) {
...
Client.prototype.publish = function (message, done) {
  console.log('client protoype publish', message)
